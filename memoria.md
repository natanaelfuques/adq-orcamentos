...
*Atualizado: 2026-04-18 (sessão 5)*
...
### index.html
- Botão **"Salvar contrato (substituir)"** (verde, acima do "Salvar contrato (novo)"): aparece só quando um contrato é carregado via "Carregar". Faz `.set()` no doc existente preservando: `criadoEm`, `criador`, `status_contrato`, `aprovado`, `autentique_id`, `link_assinatura`, `local_evento_maps`, `calendar_adicionado`, `montagem`, `desmontagem`. Após salvar, atualiza `eventos_campo` no mesmo escopo (Promise encadeada) para refletir no painel imediatamente.
- Variável `_contratoCarregadoId` — ID do contrato carregado via "Carregar" (usado pelo botão substituir)
...
| 2026-04 | Botão "Salvar contrato (substituir)" apagava contrato da lista | `.set()` sem `criadoEm` — Firestore ignora docs sem o campo usado em `orderBy`. Preservar `criadoEm` do doc original |
| 2026-04 | Maps, Calendar, montagem/desmontagem sumiam ao substituir | Campos salvos fora do formulário (`local_evento_maps`, `calendar_adicionado`, `montagem`, `desmontagem`) precisam ser preservados do doc original antes do `.set()` |
| 2026-04 | Painel não atualizava após substituir contrato | Bug de escopo: `dados` definido no 1º `.then()`, usado no 2º `.then()` (fora de escopo = `undefined`). Update em `eventos_campo` falhava silenciosamente. Solução: encadear update dentro do mesmo `.then()` onde `dados` existe |
